...
- (void)layoutSubviews {
    ...
    self.titleLb.frame = CGRectMake(CGRectGetMaxX(_imageView.frame)+5, CGRectGetMidY(_imageView.frame)-12, 100, 24);
    //self.titleLb.text = @"setting";
}
...
@property (nonatomic, strong) UserDisplayVC *userDisplayVC;

@end

static KKBookLeftSidebar *rn_frostedMenu;

@implementation KKBookLeftSidebar
...
- (instancetype)initWithImages:(NSArray *)images titles:(NSArray*)titles selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        //_itemSize = CGSizeMake(_width/4, _width/4);
        ...
        /*_userDisplayVC = [[UserDisplayVC alloc] init];
        CGRect frame = _userDisplayVC.view.frame;
        frame.size.height = 100;
        _userDisplayVC.view.frame = frame;*/
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices {
    return [self initWithImages:images titles:nil selectedIndices:selectedIndices borderColors:nil];
}

- (instancetype)initWithImages:(NSArray *)images {
    return [self initWithImages:images titles:nil selectedIndices:nil borderColors:nil];
}
...
- (void)loadView {
    ...
    //[self.view addSubview:_userDisplayVC.view];
...
- (void)layoutItems {
    ...
    CGFloat topPadding = (self.width - self.itemSize.width)/2;
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop) {
